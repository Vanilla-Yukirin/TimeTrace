...
- **动态部署模型（重要，别再误判分支"乱了"）**：重构期我们处于**动态部署**阶段，部署用 `gh workflow run deploy.yml --ref feature/refactor-split`。deploy.sh 在小主机上跑 `git reset --hard origin/<ref>`，所以**部署机本地那个名叫 `main` 的分支会指向 feature/refactor-split 的提交链**（当前可能领先 origin/main 几十个 commit）——**这是设计的镜像状态，不是污染、不是 bug、不需要"修复"**。部署机是 deployment mirror 不是 dev box，本地指向就该跟着部署 ref 走。判断真实代码状态看 `origin/*`，不看部署机本地分支名。**`origin/main` 在重构全部完工前保持 v1 legacy 不动**（pywin32 无平台 marker，Linux uv sync 会失败）；feature 完工后再正式合并并把 deploy 默认 ref 翻回 main。
