...
- [ ] **先确定根因与不变量**：一条活动记录只能覆盖客户端持续观察到的活跃时间。系统休眠、进程暂停、网络离线或 Outbox 延迟期间没有持续观测，不能用“恢复/上传时刻 - 开始时刻”冒充活动时长。
- [ ] **客户端正确终止区间**：研究 Windows suspend/resume、shutdown 和采集循环停顿；能收到 suspend/shutdown 时主动结束当前记录，resume 后必须开启新记录，不能用当前墙钟关闭休眠前的旧记录。
- [ ] **把连续观测建模为有界 lease**：记录结束时间不得晚于最后一次可信采集/心跳加一个小的容忍窗口。服务端 ingest/close 也必须校验该不变量，避免旧客户端或重放数据重新制造跨日长条。
- [ ] **运行期正确性不依赖重启**：启动扫描只保留为旧库迁移/审计工具，不再承担在线数据正确性；长期运行的服务无需重启也不能积累超长记录。
- [ ] **确定性清理历史异常**：修改生产数据前先做 SQLite 一致性快照和 dry-run 清单；仅按可解释谓词修复或删除损坏行/派生行，记录修复数量与前后不变量。无法可靠恢复的派生 VLM/摘要结果可以删除后重建，原始活动记录不得靠猜测补写。
- [ ] **清理状态机残留**：成功重试后清空旧 `error_code/error_msg/next_retry_at/locked_at`；空叙述不得标记为 `narrated`；区分“无截图所以无描述”和“有截图但分析结果为空”。
- [ ] **补齐摘要全文索引**：叙述保存、重写和删除必须同步维护 summary FTS，并为既有摘要提供幂等 backfill。
- [ ] **隐私边界**：数据质量审计不得打开或读取生产原图；疑似模型拒答记录保留原状，待 owner 后续自行确认。不得因文本规则命中而删除截图或业务记录。
