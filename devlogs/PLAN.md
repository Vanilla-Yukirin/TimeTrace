...
**最后更新：** 2026-08-30
**验证基线：** PR #9 已合并为 `d4af2f2`；Windows 客户端安装与 Release 自动发布已闭环；两台客户端的设备身份、上传与服务端入库已验证，设备维度的查询、统计与摘要仍待补齐
...
| 多设备 | 🟡 身份与入库完成、读侧未闭环 | 两个客户端使用独立 `device_id` / token / outbox 向同一后端上传；服务端已持久化设备与 `records.device_id`，普通记录 REST 可筛选；前端、MCP、Agent、搜索、统计和摘要仍缺设备维度 |
...
以下三个工作包按顺序推进。每个工作包各自完成设计、测试、文档、review 和生产验收，不合成一个难以审查和回滚的大 PR。

#### PR 1：数据质量与活动区间正确性

目标不是继续依赖“服务启动时扫描并修补异常行”，而是从活动记录的语义和状态机上消除不可信区间。

- [x] **先确定根因与不变量**：一条活动记录只能覆盖客户端持续观察到的活跃时间。系统休眠、进程暂停、网络离线或 Outbox 延迟期间没有持续观测，不能用“恢复/上传时刻 - 开始时刻”冒充活动时长。
- [x] **客户端正确终止区间**：以采集 loop 的 monotonic gap 识别 suspend/resume/进程停顿，用 gap 前最后观测墙钟收口；resume 开新记录，shutdown 同样不把未观测时间算入。
- [x] **把连续观测建模为有界 lease**：close 时间随 Outbox 原样重放；服务端写边界拒绝负数或超过 5 分钟的区间，旧客户端和重放不能重新制造跨日长条。
- [x] **运行期正确性不依赖重启**：常驻维护 loop 每分钟回收过期 open lease；`db.init()` 不再静默修改活动历史。
- [ ] **确定性清理历史异常**：修改生产数据前先做 SQLite 一致性快照和 dry-run 清单；仅按可解释谓词修复或删除损坏行/派生行，记录修复数量与前后不变量。无法可靠恢复的派生 VLM/摘要结果可以删除后重建，原始活动记录不得靠猜测补写。
- [x] **清理状态机残留**：成功重试清空旧 `error_code/error_msg/next_retry_at/locked_at`；VLM 空文本不再成功，有截图的历史空结果可重排队，无截图正常 skip 和无 `vlm_model` 的手工权威标签均明确排除；空叙述由显式修复重排队。
- [x] **补齐摘要全文索引**：叙述保存/重写使用 delete+insert 保持单一当前 FTS 行；显式修复幂等重建既有摘要索引。
- [x] **隐私边界**：审计和修复只查关系/状态元数据，不打开、不读取、不删除原图；疑似模型拒答记录保留原状，待 owner 后续自行确认。
- [ ] **测试门槛**：覆盖休眠十几小时后恢复、Outbox 延迟上传/重放、客户端崩溃、服务端长期不重启、成功重试、空模型输出和重复修复幂等；验证时间轴不再产生跨日长条。

#### PR 2：设备查询闭环与多设备时间轴

不传 `device_id` 时继续保持“全部设备”兼容行为；传 UUID 时限定单台设备；迁移前无身份记录统一使用 `_unknown_device`，不猜测归属。

- [ ] **统一设备目录**：复用服务端设备注册表，向 REST、MCP、Agent 和前端返回稳定 `device_id`、可改设备名称、设备描述、last seen 与必要能力信息；设备名称/描述不复制进业务记录。
- [ ] **查询与搜索**：记录、文本搜索、多模态搜索、最近活动均在候选检索阶段应用设备范围，并在结果中返回设备身份，不能先做全局 top-N 再事后过滤。
- [ ] **统计**：app/category/query_stats 支持设备筛选。全设备 `active_seconds` 保持跨设备墙钟并集；另行明确各设备时长相加的口径，不能把二者都叫“实际经过时间”。
- [ ] **MCP 与 Agent**：增加 `list_devices`；所有相关工具接收可选 `device_id`。Agent 提示上下文包含设备名称与描述，使“办公电脑”“家用电脑”等语义能参与回答；`ask_agent` 必须把设备范围一路传入真实检索。
- [ ] **前端选择器和设备列**：时间轴、搜索、统计页面提供“全部 / 单设备 / 未知设备”选择；记录列表显示设备列或 badge，筛选状态进入 URL 与 React Query cache key。
- [ ] **多设备日时间轴**：同一日期使用共享横向时间刻度，每台设备占一条独立 lane；所有 lane 同步横向缩放/滚动，设备较多时容器支持鼠标滚轮纵向滚动。重叠活动保持平行显示，使交错使用与同时使用可直接比较。
- [ ] **测试门槛**：至少覆盖两台设备 + legacy NULL、不传过滤器、单设备、未知设备、非法/不存在 UUID、同一时刻双设备活动、设备改名、撤销后的历史可读性，以及双进程 smoke 最终归属断言。

#### PR 3：设备级摘要金字塔 v2（暂缓，不在下一次实现）

当前全局摘要已经混合多台设备，无法在读取阶段可靠拆开。该工作包等 PR 1、PR 2 稳定后再启动。

- [ ] 新建并行 `device_summaries` v2 派生表，唯一身份包含 `(grain, scope_key, device_scope)`；保留旧 `summaries`，避免 schema 迁移破坏旧 Docker 镜像的自动回滚。
- [ ] 每个设备和 `_unknown_device` 各自生成 `5min → 1h → 6h → day → week` 金字塔；叶子指标、叙述输入、父子合并、pending gate 和 source hash 全部限定同一设备。
- [ ] 默认摘要查询返回每台设备各自的叙述，不再生成虚假的混合故事；旧客户端所需的顶层字段保持兼容，新响应补充设备字段与可选分组结构。
- [ ] 历史 v2 从原始记录重新 backfill，不能把旧全局叙述直接改名成设备叙述；未回填窗口可短期回退旧聚合摘要，但新旧结果不得重复返回。
- [ ] LM Studio 单卡 `parallel=1` 下按设备公平调度，监控 `pending_summary`，避免活跃设备长期挤占其他设备；叙述量随设备数近似线性增长，需要独立容量验收。

上述三个 PR 的当前顺序是 **数据正确性 → 查询与展示 → 设备摘要派生层**。PR 3 已记录但明确暂缓。
