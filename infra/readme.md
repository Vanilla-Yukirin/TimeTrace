...
- **`privacy/strategy.md` 暂未加警告**：内容仍是 v1 规则黑名单 + 服务端过滤 —— 此刻代码就是这样跑的，描述准确；P4 OCR 管线落地（客户端 OCR + 模糊、原图不离机）时一并加警告 + 整页重写。在那之前直接读即可
- **完整状态优先看 [kickoff devlog](../devlogs/infra/archive-202605151200-client-server-split-kickoff.md)**：那是滚动更新的"重构宪法"
- **新增 deprecation 警告的格式约定**：H2 + 加粗 emoji，即 `## **⚠️ 一句话标题**`（双星紧贴 emoji，无空格）。这样 `grep '^## \*\*⚠️'` 才能稳定命中
